---
title: 'mysql概述二'
date: 2021-05-19 17:20:46
tags: [mysql]
published: true
hideInList: false
feature: /post-images/QzFPAEzVT.png
isTop: false
---
### 索引
...
![](https://tinaxiawuhao.github.io/post-images/1620897817779.png)
...
![](https://tinaxiawuhao.github.io/post-images/1620897795644.png)
...
![](https://tinaxiawuhao.github.io/post-images/1620897774703.png)
...
BTree是最常用的mysql数据库索引算法，也是mysql默认的算法。因为它不仅可以被用在`=`,`>`,`>=`,`<`,`<=`和`between`这些比较操作符上，而且还可以用于`like`操作符，只要它的查询条件是一个不以通配符开头的常量， 例如：

-- 只要它的查询条件是一个不以通配符开头的常量 `select * from user where name like 'jack%'; `
...
1. B树索引
   ...
   mysql通过存储引擎取数据，基本上90%的人用的就是InnoDB了，按照实现方式分，InnoDB的索引类型目前只有两种：BTREE（B树）索引和HASH索引。B树索引是Mysql数据库中使用最频繁的索引类型，基本所有存储引擎都支持BTree索引。通常我们说的索引不出意外指的就是（B树）索引（实际是用B+树实现的，因为在查看表索引时`SHOW INDEX FROM <表名> [ FROM <数据库名>]`，mysql一律打印BTREE，所以简称为B树索引）

   ![](https://tinaxiawuhao.github.io/post-images/1620984450233.png)

   **查询方式：**

   主键索引区:PI(关联保存的数据的地址)按主键查询,
...
![](https://tinaxiawuhao.github.io/post-images/1620897738341.webp)
...
![](https://tinaxiawuhao.github.io/post-images/1620897723431.jpeg)
...
![](https://tinaxiawuhao.github.io/post-images/1620897704953.png)
...
> 漫画：什么是B+树？https://tinaxiawuhao.github.io/post/jVTlR3ljT/